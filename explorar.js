...
function normalizeItem(ep) {
    return {
        ...
        initialMode: ep.initialMode || ep.type || 'audio',
        ...
        bgColor: ep.bgColor || '#0a0a0a',
        subtitlesUrl: ep.subtitlesUrl || '',
        text: ep.text || ''
    };
}

// Verifica si un episodio está en la playlist global (userStorage)
function checkInPlaylist(epId) {
    return userStorage.playlist.has(epId);
}

// Actualiza el icono de añadir en todos los botones que apunten al mismo episodio (por ID)
function updateAddButtons(epId) {
    document.querySelectorAll(`[data-ep-id="${epId}"] img[data-added], [data-ep-id="${epId}"] .np-icon-add`).forEach(img => {
        const inPlaylist = checkInPlaylist(epId);
        if (img.tagName === 'IMG') {
            img.src = inPlaylist ? ICONS.added : ICONS.add;
            img.setAttribute('data-added', inPlaylist ? 'true' : 'false');
        }
    });
}
...
function renderHero() {
    ...
    heroItems.forEach((item, idx) => {
        ...
        const inPlaylist = checkInPlaylist(item.id);
        ...
        slidesHtml += `
            <div class="np-hero-slide ${activeClass}" data-index="${idx}" onclick="window.explorar.goToDetail('${item.detailUrl}')">
                <div class="np-hero-content" onclick="event.stopPropagation();">
                    <h1 onclick="event.stopPropagation(); window.explorar.goToDetail('${item.detailUrl}')">${escapeHtml(item.title)}</h1>
                    <div class="author" onclick="event.stopPropagation(); window.explorar.goToDetail('${item.detailUrl}')">${escapeHtml(item.author)}</div>
                    <div class="np-hero-controls" onclick="event.stopPropagation();">
                        <div class="np-hero-controls-left">
                            <button onclick="event.stopPropagation(); window.explorar.downloadItem('${item.id}')" title="Descargar">
                                <img src="${item.allowDownload ? ICONS.dl : ICONS.noDl}">
                            </button>
                            <button onclick="event.stopPropagation(); window.explorar.shareItem('${escapeHtml(item.title)}', '${item.detailUrl}')" title="Compartir">
                                <img src="${ICONS.share}" style="filter: brightness(0) invert(1);">
                            </button>
                            <button onclick="event.stopPropagation(); window.explorar.togglePlaylist('${item.id}')" title="Añadir a lista">
                                <img src="${addIcon}" data-ep-id="${item.id}" data-added="${inPlaylist}">
                            </button>
                        </div>
                        <button class="np-hero-play-btn" onclick="event.stopPropagation(); window.explorar.playItem('${item.id}')">
                            <img src="${ICONS.play}"> Reproducir
                        </button>
                    </div>
                </div>
                <div class="np-hero-image">
                    <img src="${item.coverWide || item.cover}" alt="${escapeHtml(item.title)}" loading="lazy">
                </div>
            </div>
        `;
...
function renderSeriesDestacadas() {
    ...
    seriesArray.forEach(serie => {
        ...
        html += `
            <div class="np-series-item" style="padding-left:0;" onclick="window.explorar.goToDetail('${s.url_serie || '#'}')">
                ...
                <div style="flex:1; min-width:0;">
                    <div class="np-title">${escapeHtml(s.titulo_serie)}</div>
        ...
        episodes.forEach((ep, idx) => {
            const inPlaylist = checkInPlaylist(ep.id);
            const addIcon = inPlaylist ? ICONS.added : ICONS.add;
            
            html += `
                <div class="np-series-item" data-ep-id="${ep.id}">
                    ...
                    <div class="np-series-thumb" onclick="window.explorar.goToDetail('${ep.detailUrl}')">
                        ...
                        <div class="np-overlay-mini" onclick="event.stopPropagation(); window.explorar.playItem('${ep.id}'); return false;">
                            <img src="${ICONS.play}" class="np-play-icon-sm">
                        </div>
                    </div>
                    <div style="flex:1; min-width:0;" onclick="window.explorar.goToDetail('${ep.detailUrl}')">
                        <div class="np-title">${escapeHtml(ep.title)}</div>
                        <div class="np-author">${escapeHtml(ep.author)}</div>
                    </div>
                    <button class="np-add-btn" style="background:none; border:none; cursor:pointer;" 
                            onclick="event.stopPropagation(); window.explorar.togglePlaylist('${ep.id}'); return false;">
                        <img src="${addIcon}" style="width:20px; height:20px; filter: brightness(0) invert(1);" data-ep-id="${ep.id}" data-added="${inPlaylist}">
                    </button>
                </div>
            `;
        });
...
function renderVideosDestacados() {
    ...
    const videos = DATA
        .filter(ep => ep.initialMode === 'video' || ep.type === 'video')
    ...
    videos.forEach(ep => {
        ...
        html += `
            <div class="np-card-video group" data-ep-id="${ep.id}">
                ...
                <div class="np-overlay-full">
                    <img src="${addIcon}" class="np-icon-add" onclick="event.stopPropagation(); window.explorar.togglePlaylist('${ep.id}'); return false;" data-ep-id="${ep.id}" data-added="${inPlaylist}">
                    <img src="${ICONS.play}" class="np-icon-play" onclick="event.stopPropagation(); window.explorar.playItem('${ep.id}'); return false;">
                    <img src="${ep.allowDownload ? ICONS.dl : ICONS.noDl}" class="np-icon-dl" onclick="event.stopPropagation(); window.explorar.downloadItem('${ep.id}'); return false;">
                </div>
                <span class="np-mobile-play" onclick="event.stopPropagation(); window.explorar.playItem('${ep.id}'); return false;">
                    <img src="${ICONS.play}">
                </span>
...
function renderRandomGrid() {
    ...
    selected.forEach(ep => {
        ...
        html += `
            <div class="np-card-std" data-ep-id="${ep.id}">
...
// ========== ACCIONES GLOBALES (INTEGRADAS CON player.js) ==========

// Reproducir usando la función global del player
function playItem(epId) {
    const ep = DATA.find(e => e.id === epId);
    if (!ep) return;

    // Llamar a la función global expuesta por player.js (igual que en show.js)
    if (typeof window.playEpisodeExpanded === 'function') {
        window.playEpisodeExpanded(
            ...
            ep.detailUrl || '',
            ep.author || '',
            getAllEpisodios(),        // lista completa de episodios (para cola)
        ...
        console.error('playEpisodeExpanded no está definida. Asegúrate de que player.js está cargado.');
        window.open(ep.mediaUrl, '_blank');
    }
}

// Descargar usando lógica similar a window.handleDl
function downloadItem(epId) {
    const ep = DATA.find(e => e.id === epId);
    if (!ep) return;
    if (!ep.allowDownload) {
...
// Añadir / quitar de playlist usando userStorage (global)
function togglePlaylist(epId) {
    const ep = DATA.find(e => e.id === epId);
    if (!ep) return;
    ...
    // Actualizar todos los botones visuales de este episodio
    updateAddButtons(epId);
}

// Compartir usando la función global de show.js (si existe) o implementación local
function shareItem(title, url) {
    if (typeof window.shareContent === 'function') {
        const ep = DATA.find(e => e.detailUrl === url);
        window.shareContent(title, url, ep?.coverUrl || '', ep?.description || '');
    } else {
...
// Navegación a detalle usando la función global (evita recarga)
...
export function render(container) {
    // Cargar datos (asegurar que cada episodio tenga id numérico/string y datos completos)
    DATA = getEpisodiosConSerie().map(ep => ({
        ...ep,
        id: ep.id || ep.mediaUrl, // garantizar id único
        initialMode: ep.initialMode || ep.type || 'audio',
        bgColor: ep.bgColor || '#0a0a0a'
    }));
