// show/detail.js - Vistas de detalle de episodio y serie (con scroll top y registro)
import { DATA, addToHistory, getPersonalizedRecommendations } from './utils.js';
import { ICONS } from './constants.js';
import { userStorage } from '../storage.js';
import { createCarousel } from './carousel.js';

// Función auxiliar para crear carrusel de recomendaciones personalizadas
function createRecommendedCarousel(currentEp) {
    const recommendations = getPersonalizedRecommendations(12, currentEp.id);
    if (recommendations.length === 0) return '';
    return createCarousel("Recomendaciones para ti", "standard", recommendations, "Todos", 'items');
}
...
export function renderEpisodio(container, episodioId) {
    ...
    try {
        ...
        // Registrar visualización para personalización
        addToHistory(ep.id);
...
export function renderSerie(container, serieUrl) {
    ...
    try {
        ...
        // Registrar visualización del primer episodio como interacción
        if (episodiosSerie.length > 0) addToHistory(episodiosSerie[0].id);
